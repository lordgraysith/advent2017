...
const input = `3751	3769	2769	2039	2794	240	3579	1228	4291	220	324	3960	211	1346	237	1586
...
function run () {
  const result = R.pipe(
    R.split('\n'),
    R.map(lineToArray),
    R.map(arrayToNumbersArray),
    R.map(numberArrayToDividedPairs),
    R.map(x => x.answer),
    R.sum()
  )(input)
  console.log(result)
...
const lineToArray = R.split('\t')
const arrayToNumbersArray = R.map(x => +x)
const numberArrayToDividedPairs = R.reduce(
  (previousValue, currentValue) => {
    if (typeof previousValue.answer === 'number') return previousValue
    const numberThatDividesThis = getNumberThatDividesThis(
      currentValue,
      previousValue.anArray
    )
    const anArray = previousValue.anArray.concat([currentValue])
    if (typeof numberThatDividesThis === 'number') {
      return { answer: numberThatDividesThis, anArray }
    }
    return { anArray }
  },
